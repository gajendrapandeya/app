...
public class JSONData {
    ...
    public static String getStringDefNull(JSONObject item, String key) {
        try {
            return item.has(key) ? (item.isNull(key) ? null : item.getString(key)): null;
        } catch (Exception e) {
//            e.printStackTrace();
        }
        return null;
    ...
    public static String getStringDefDash(JSONObject item, String key) {
        try {
            return item.has(key) ? (item.isNull(key) ? "-" : item.getString(key))
                    : "-";
        } catch (Exception e) {
    ...
    public static String getStringPrice(JSONObject item, String key) {
        try {
            return item.has(key) ? (item.isNull(key) ? "0" : item.getString(key))
                    : "0";
        } catch (Exception e) {
            //e.printStackTrace();
        }
